  ...
  methods: {
    ...
    weekChange(event) {
      ...
      if (event.dateScope && event.dateScope.endDate) {
        const endDate = this.formatDate(event.dateScope.endDate);
        this.getGlobalIndex(endDate);
      }
    },
    getGlobalIndex(endDate) {
      const self = this;
      API.getGlobalIndex(endDate).then(
        res => {
          self.xData = res.date;
