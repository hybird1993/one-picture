...
export default {
  ...
  mounted() {
    this.getSpecialPeopleList(true);
  },
  destroyed() {
    const self = this;
    if (self.timer) {
  ...
  methods: {
    getSpecialPeopleList(isLoop = false) {
      const self = this;
      API.getSpecialPeopleList().then(
        res => {
          // console.log(res);
          self.allList = res.data;
          self.allList.forEach((item, index) => {
            item.index = index + 1;
            item["isAlreadyDeal"] = item.status === "已处理";
          });

          if (isLoop) {
            self.list = [].concat(self.allList);
            self.loop();
          } else {
            self.list.forEach(item => {
              item = self.allList[item.index - 1];
            });
          }
          self.loopTimer = setTimeout(() => {
            self.getSpecialPeopleList();
          }, LOOP_TIME);
        },
        err => {
          self.loopTimer = setTimeout(() => {
